...
class OpenAIProvider(BaseProvider):
    """Provider for OpenAI chat completion APIs."""
        ...
        with requests.post(
            ...
            for line in resp.iter_lines():
                ...
                if line.startswith("data: "):
                    line = line[6:]
                if line == "[DONE]":
                    yield StreamChunk(delta="", model=resolved_model, provider="openai", is_final=True)
                    return
                try:
    ...
    def _raise_for_status(resp: requests.Response) -> None:
        if resp.status_code == 401:
            raise AuthenticationError("Clé API OpenAI invalide ou manquante.")
        if resp.status_code == 429:
            raise RateLimitError("Limite de débit OpenAI atteinte. Réessayez plus tard.")
        if resp.status_code >= 400:
            try:
                detail = resp.json().get("error", {}).get("message", resp.text)
            except Exception:
                detail = resp.text
            raise APIError(f"Erreur OpenAI {resp.status_code} : {detail}")